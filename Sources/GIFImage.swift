...
public struct GIFImage: View {
    ...
    @EnvironmentObject @MainActor private var store: PresentationStore
    ...
    @Sendable private func load() {
        ...
        presentationTask = Task {
            await presentationController.start(
                store: store,
...
let gifURLString = "https://raw.githubusercontent.com/igorcferreira/GIFImage/main/Tests/test.gif"
let gifURL = URL(string: gifURLString)!
#Preview("Raw URL") {
    GIFImage(source: .remoteURL(gifURL), placeholder: placeholder, errorImage: error)
}
#Preview("Limited 5 FPS") {
    GIFImage(source: .remoteURL(gifURL), placeholder: placeholder, errorImage: error, frameRate: .limited(fps: 5))
}
#Preview("Limited to 30 FPS") {
    GIFImage(source: .remoteURL(gifURL), placeholder: placeholder, errorImage: error, frameRate: .static(fps: 30))
}
#endif
